...
if (!class_exists('Local_Pickup_Integration')) {
	...
	class Local_Pickup_Integration implements IntegrationInterface
	{
		...
		private function register_main_integration()
		{
			...
			wp_register_script(
				'pickup-checkout-blocks-integration',
		...
		{
			return array('pickup-checkout-blocks-integration', 'pickup-checkout-block-frontend');
		}
		/**
		...
		{
			return array('pickup-checkout-blocks-integration', 'pickup-checkout-block-editor');
		}
		/**
...
// 			);
			...
			if ( !empty($_COOKIE['wcmlim_selected_location_termid']) ) {
				...
				$start_time = get_term_meta($location_id, 'wcmlim_start_time', true);
				$end_time = get_term_meta($location_id, 'wcmlim_end_time', true);
				if ( empty($start_time) || empty($end_time) ) {
					...
					for ($time = $start; $time <= $end; $time += 60 * 60) {
						$formatted_time = date('H:i', $time);
                        // if ( $end <= ($time += 60 * 60) ) {
                            $time_interval[] =  $formatted_time;
                        // } else {
                        //     $time_interval[] = sprintf('%s - %s', $formatted_time, date('H:i', ($time += 60 * 60)));
                        // }
					}
				}
			}
		...
		{
			$script_asset_path = WC_PICKUP_DIR . '/build/pickup-checkout-block.asset.php';
			$script_asset = file_exists($script_asset_path)
				? require $script_asset_path
				: array(
					'dependencies' => array('wp-data'),
			...
			wp_register_script(
				'pickup-checkout-block-editor',
				WC_PICKUP_URL . 'build/pickup-checkout-block.js',
				$script_asset['dependencies'],
				$script_asset['version'],
				true
			);
		...
		{
			$script_asset_path = WC_PICKUP_DIR . '/build/pickup-checkout-block-frontend.asset.php';
			$script_asset = file_exists($script_asset_path)
				? require $script_asset_path
				: array(
					'dependencies' => array('wp-data'),
			...
			wp_register_script(
				'pickup-checkout-block-frontend',
				WC_PICKUP_URL . 'build/pickup-checkout-block-frontend.js',
				$script_asset['dependencies'],
				$script_asset['version'],
				true
			);
