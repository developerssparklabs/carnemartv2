<?php 
namespace WC_MMQ_PRO\Framework;
...
if( ! class_exists( 'WC_MMQ_PRO\Framework\Required' ) ){
    ...
    class Required
    {
        ...
        $disable_plugin_noti = !isset( $config['disable_plugin_noti'] ) ? true : false;
            ...
            $r_slug = 'woo-min-max-quantity-step-control-single/wcmmq.php';
            $t_slug = 'WC_Min_Max_Quantity/wcmmq.php';
            $req_wc = new Require_Control($r_slug,$t_slug);
            $req_wc->set_args(['Name'=>'Min Max Quantity & Step Control for WooCommerce'])
            ->set_download_link('https://wordpress.org/plugins/woo-min-max-quantity-step-control-single/')
            ->set_this_download_link('https://codeastrology.com/min-max-quantity/')
