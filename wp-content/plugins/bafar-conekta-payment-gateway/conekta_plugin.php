...
class WC_Conekta_Plugin extends WC_Payment_Gateway
{
    ...
    public static function check_if_payment_payment_method_webhook(string $payment_method, array $event){
        ...
        // Verifica si 'metadata' y 'payment_method' existen
        if (isset($conekta_order["metadata"]) && isset($conekta_order["metadata"]["payment_method"])) {
            ...
            // Maneja el caso en que 'payment_method' no esté definido
            header('Content-Type: application/json');
            echo json_encode(['message' => 'Error: payment_method no definido']);
            exit;
        }
    }
