...
function ckpg_build_line_items($items, $version)
{
    ...
    foreach ($items as $item) {
        ...
        $sub_total   = floatval($item['line_subtotal']);
        ...
        if (fmod($quantity, 1) > 0) {
            // Forzar cantidad a 1 y recalcular el precio unitario
            $unit_price = intval(round($sub_total * 100)); // Subtotal en centavos
            $quantity = 1; // Forzar cantidad a 1
        } else {
            // Calcular el precio unitario normal
            $unit_price = intval(round(($sub_total / $quantity) * 100)); // Subtotal dividido por cantidad en centavos
        }
...
function ckpg_get_request_data($order)
{
    ...
    // Procesar los fees del pedido estos se hadcodearon para enviarlos a conekta sparklabs sergio keikos s8k
    $fees = $order->get_items('fee');

    foreach ($fees as $fee_id => $fee) {
        $fee_name = $fee->get_name();

        // Omitir si el nombre del fee contiene "Costo de Envío"
        if (strpos($fee_name, 'Costo de Envío') !== false) {
            continue; // Salta este fee
        }

        // Agregar el fee como descuento si no contiene "Costo de Envío"
        $discount_lines = array_merge($discount_lines,
            array(
                array(
                    'code'   => $fee_name,
                    'type'   => 'coupon', // Definir el tipo como 'coupon' para los fees
                    'amount' => abs(round($fee->get_total() * 100))
                )
            )
        );
    }
        ...
        //preguntar si hay algun costo de envío extra sergio s8k keikos spark
        // Procesar los fees del pedido y ver si tenemos uber o rappi o cualquier deliver.
        $fees = $order->get_items('fee');
        foreach ($fees as $fee_id => $fee) {
            $fee_name = $fee->get_name();
            // Omitir si el nombre del fee contiene "Costo de Envío"
