    ...
    protected function processOpenpayCharge($device_session_id, $openpay_token, $payment_plan, $use_card_points, $openpay_cc, $save_cc, $card_number)
    {
        ...
        if ($charge != false) {
            ...
            if ($charge_request['capture'] == false && $charge->status == 'in_progress') {
                ...
                /**
                 * Petición a ClearSales con Openpay
                 */
                try {
                    // Para probar metemos aquí la parte de multilocations
                    $location_id = !empty($_COOKIE['wcmlim_selected_location_termid']) ? $_COOKIE['wcmlim_selected_location_termid'] : '';
                    $centro = !empty($location_id) ? get_term_meta($location_id, 'centro_location', true) : '';
                    $location = !empty($_COOKIE['wcmlim_selected_location']) ? $_COOKIE['wcmlim_selected_location'] : '';
                    ...
                    if (!empty($location_id)) {
                        $this->order->update_meta_data('location_id', $location_id);
                    }

                    if (!empty($centro)) {
                        $this->order->update_meta_data('centro', $centro);
                    }

                    if (!empty($location_name)) {
                        $this->order->update_meta_data('location_name', $location_name);
                    }

                    if (!empty($location)) {
                        $this->order->update_meta_data('location', $location);
                    }

                    $this->order->save();


                    require_once __DIR__ . '/ClearSalesOpenpay.php';
                    $clearSalesOpenpay = new ClearSalesOpenpay();
                    $response_clearSales = $clearSalesOpenpay->send_order($this->order->get_id(), $charge);
                    if ($response_clearSales[0]) {
                        $this->order->update_meta_data('enviado_clear_sales', 1);
                        $this->order->save();
                        $this->order->add_order_note($response_clearSales[1], false);
                        $this->order->update_status('pending', $response_clearSales[1], false);
                    } else {
                        $this->order->add_order_note($response_clearSales[1], false);
                        $this->order->update_status('pending', $response_clearSales[1], false);
                    }
