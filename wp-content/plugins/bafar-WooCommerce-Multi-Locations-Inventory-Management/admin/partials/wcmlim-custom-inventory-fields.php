...
class Wcmlim_Custom_Inventory_Fields
{
    ...
    if ($product_type == "simple") {
      ...
      if ($defaultLocation == "on" && $backendonlymode != "on" && $stockmanagment == "yes") {
        ...
        foreach ($terms as $key => $term) {
          $options["loc_{$term->term_id}"] = $term->name;
        }
      ...
      foreach ($terms as $term) {
        ...
        $stock = trim($stock); // Elimina espacios en blanco al inicio y final
  ...
  public function wcmlim_save_custom_inventory_fields($post_id)
  {
    ...
    $locations_check = isset($_POST['tax_input']['locations']) ?? '';
    ...
    if ($product->is_type('simple')) {
      ...
      foreach ($terms as $term) {
        ...
        if (isset($_POST['wcmlim_product_' . $post_id . '_location_id_' . $term->term_id])) {
          ...
          if ($counter === $terms_total) {
            $total_stock = 0;
            foreach ($input_amounts as $key => $value) {
              if (is_numeric($value)) {
                $total_stock += $value;
              }
            }
          ...
          $stock = get_post_meta($post_id, "wcmlim_stock_at_{$term->term_id}", true);
          $_parentStock[] = is_numeric($stock) ? (float) $stock : 0;
      ...
      foreach ($product_terms_stock as $key => $value) {
        if (is_numeric($value)) {
          $total_terms_stock += $value;
        }
      }
      $product_terms_stock = $total_terms_stock;

      $total_stock_qty = 0;
      foreach ($input_amounts as $key => $value) {
        if (is_numeric($input_amounts)) {
          $total_stock_qty += $value;
        }
      }
    ...
    foreach ($terms as $term) {
      ...
      if ($backendonlymode == 'on') {
        ...
        <p class="form-field <?php echo "wcmlim_variation_{$variation->ID}_cogs_at_{$term->term_id}_field" ?>">
          <label for="<?php echo "wcmlim_variation_{$variation->ID}_cogs_at_{$term->term_id}_field" ?>">Purchase price
            ($)</label>
          <span class="woocommerce-help-tip"></span>
          <input type="text" class="woocommerce wcmlim_variable_cogs_at" style=""
            name="<?php echo "wcmlim_variation_{$variation->ID}_cogs_at_{$term->term_id}_field" ?>"
            id="<?php echo "wcmlim_variation_{$variation->ID}_cogs_at_{$term->term_id}" ?>"
            value="<?php echo get_post_meta($variation->ID, "wcmlim_cogs_at_{$term->term_id}", true); ?>" placeholder="">
        </p>
        <?php
        echo '</div>';
      }
